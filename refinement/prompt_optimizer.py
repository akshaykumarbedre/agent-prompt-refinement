...
from core.models import PromptVersion, EvaluationResult, HumanAnnotation
...
class PromptOptimizer:
    ...
    OPTIMIZER_SYSTEM_PROMPT = """You are an expert prompt engineer. Your task is to improve an AI agent's system prompt based on evaluation failures.

You will receive:
1. The current system prompt
2. A failure analysis summary
3. Example failures with feedback
4. Human annotations (if available)

Your job is to:
1. Understand the root causes of failures
2. Modify the prompt to address these issues
3. Preserve what's working well
4. Apply specific fix strategies
...
Respond ONLY with a JSON object:
{
    ...
    OPTIMIZER_USER_TEMPLATE = """Improve this prompt based on the failure analysis:
...
Generate an improved prompt that addresses these issues."""
    ...
    def _format_human_feedback(self, annotations: Optional[list[HumanAnnotation]]) -> str:
        """Format human annotations for the optimizer."""
        if not annotations:
        ...
        for ann in annotations:
            if ann.rating == "bad":
                ...
                if ann.suggested_fix:
                    lines.append(f"  Suggested fix: {ann.suggested_fix}")
        
        return "\n".join(lines) if lines else "No negative feedback to address."
